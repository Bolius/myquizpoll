...
/*							if ($this->conf['isPoll']) {	// wurde aus der Schleife genommen!
				...
				if (is_array($statisticsArray)) {
					foreach ($statisticsArray as $type => $element) {
						...
						$insert = array('pid' => $resPID,
										...
										'nextcat' => $element['nextCat'],
										'sys_language_uid' => $this->lang);
						//$array2=array();
						for( $answerNumber=1; $answerNumber < $this->answerChoiceMax+1; $answerNumber++ ) {
							$insert['checked'.$answerNumber] = $element['a'.$answerNumber];
						}
			...
			if ( $numOfQuestions > 0 ) {		// any questions out there???
				...
				for ($questionNumber=1; $questionNumber < $maxQuestions+1; $questionNumber++) {
					...
					for( $answerNumber=1; $answerNumber <= $this->answerChoiceMax; $answerNumber++) {
						...
						if( (!$text_type && ($row['answer'.$currentValue] || $row['answer'.$currentValue]==='0')) || ($text_type && $answerNumber==1) ) {
							...
							if ($row['qtype'] == 1) {	// radio-button
								...
								if ($markerArrayQ["###MY_INPUT_LABEL###"]) { $input_label1 = '<label for="answer'.$questionNumber.'_'.$answerNumber.'_1">'; $input_label2 = '</label>'; }
								...
								if (is_array($oldRelData[$quid]) && $oldRelData[$quid]['checked'.$currentValue]) $answer_content .= ' checked="checked"';
								$answer_content .= ' /> '.$input_label1.$this->pi_getLL('yes','yes').$input_label2.'</span>';
								if ($markerArrayQ["###MY_INPUT_ID###"]) $input_id = 'id="answer'.$questionNumber.'_'.$answerNumber.'_0"';
								if ($markerArrayQ["###MY_INPUT_LABEL###"]) { $input_label1 = '<label for="answer'.$questionNumber.'_'.$answerNumber.'_0">'; $input_label2 = '</label>'; }
								...
								if (is_array($oldRelData[$quid]) && !$oldRelData[$quid]['checked'.$currentValue]) $answer_content .= ' checked="checked"';
								$answer_content .= ' /> '.$input_label1.$this->pi_getLL('no','no').$input_label2.'</span></span>';
					...
					if ($this->conf['allowSkipping']) {		// skip answers
						...
						// Questtion type
						if ($row['qtype'] == 1) {
							$answer_content = '<input type="radio" name="tx_myquizpoll_pi1[answer'.$questionNumber.']" value="-1" /> '.$this->pi_getLL('skip','skip');
						} else if ($row['qtype'] == 2) {
							$answer_content = '<option value="-1"> '.$this->pi_getLL('skip','skip').'</option>';
						} else if ($row['qtype'] < 6) {
							$answer_content = '<input type="checkbox" name="tx_myquizpoll_pi1[answer'.$questionNumber.']" value="-1" /> '.$this->pi_getLL('skip','skip');
						}
  ...
  <input type="hidden" name="'.$this->prefixId.'[joker3]" value="0" />';
	...
	function showPollResult($answered, $quizData, $thePID,$resPID) {
		...
		$markerArray["###VAR_QUESTION_NAME###"] = $this->formatStr($row['name']); // $this->pi_RTEcssText($row['name']);
		if ($withStartCat) {	// nur antworten zur frage einer kategorie nehmen
			$answeredQ = " AND $dbquestion=".$row['uid'];
		}
		if ($row['image']) {
			$template_image_begin = $this->cObj->getSubpart($this->templateCode, "###TEMPLATE_QUESTION_IMAGE_BEGIN###");
			$template_image_end = $this->cObj->getSubpart($this->templateCode, "###TEMPLATE_QUESTION_IMAGE_END###");
